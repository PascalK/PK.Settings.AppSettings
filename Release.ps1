...
Write-Host $Root
$Solution = Get-ChildItem $Root "PK.Settings.AppSettings.sln" -File
...
$PKSettingsAppSettings = Get-ChildItem $Sources "PK.Settings.AppSettings.Net45" -Directory
Write-Host $PKSettingsAppSettings.FullName
$PKSettingsAppSettingsProj = Get-ChildItem $PKSettingsAppSettings.FullName "PK.Settings.AppSettings.Net45.csproj" -File
Write-Host $PKSettingsAppSettingsProj.FullName

#goto :eof
& $NuGet restore $Solution.FullName

& msbuild /t:Pack /p:Configuration=Release $PKSettingsAppSettingsProj.FullName /p:NuGetExePath=$NuGet